module.exports = function (eleventyConfig) {
  ...
  eleventyConfig.addLiquidShortcode("tmdbPoster", async (title, year, size = "w154") => {
    if (!tmdbKey) return "";
  ...
  eleventyConfig.addFilter("lastModified", (page) => fs.statSync(page.inputPath).mtime.getTime());
  ...
  eleventyConfig.addTransform("lazyImages", function (content, outputPath) {
    if (outputPath && outputPath.endsWith(".html"))
      return content.replace(/<img(?!.*loading=)(.*?)>/g, '<img$1 loading="lazy">');
    return content;
  });
  ...
  eleventyConfig.addPassthroughCopy("assets", {
    ...
    filter: ["**/*", "!img/**/*.jpg", "!img/**/*.jpeg", "!img/**/*.png", "img/dither/**/*"],
  });
  ...
  async function generateDitheredImage(inputPath, outPath, maxWidth = null) {
    ...
    for (let y = 0; y < height; y++)
      for (let x = 0; x < width; x++) {
        ...
        const color = lum > normalizedThreshold ? DUOTONE_PALETTE[0] : DUOTONE_PALETTE[1];
        data[idx] = color.r;
        data[idx + 1] = color.g;
        data[idx + 2] = color.b;
      }
  ...
  return {
    dir: { input: "content", output: "_site", layouts: "../layouts", includes: "../includes", data: "../_data" },
