...
function firstMediaSrc(item) {
	if (!item || !Array.isArray(item.medias) || !item.medias.length) return "";
	return String(item.medias[0].src || "");
}
...
function slugFromItem(item, index) {
	...
	const byFileName = slugify(fileName.replace(/\.[a-z0-9]+$/i, ""));
...
function getGalleryDefinitions() {
	...
	const files = fs
		...
		.filter((name) => /\.(md|njk)$/i.test(name));
	const defs = [];
